...
|---|---|---|---|
| 1 | User | เลือกรายการ + จำนวน → เลือก **Budget + เดือน ของแต่ละรายการ** (หรือ Budget ตั้งต้นใช้กับทุกรายการ) · ใส่ขอบเขตการใช้งาน · Budget/เดือนที่งบไม่พอ ต้องติ๊ก **ขอ Over Budget** พร้อมเหตุผล ไม่งั้นส่งไม่ได้ | รอ Admin ตรวจ |
| 2 | Admin | ตรวจคำขอ → **ส่งอีเมลขออนุมัติผู้บริหาร** (แก้ ถึง/สำเนา/เรื่อง/เรียน/ข้อความ และดูตัวอย่างก่อนส่ง) — เนื้อหาตามรูปแบบเดิม: Budget เดือน…, งบตาม BG vs ค่าใช้จ่ายโดยประมาณ (ไม่รวม VAT), แยก "สื่อ … หมวด…" → งบ BG รายเดือน → รายการ, ยอดคงเหลือ STORE, หมายเหตุ, ตาราง 14 คอลัมน์, รายการ Over Budget + เหตุผล | รอผู้บริหารอนุมัติ |
| 3 | ผู้บริหาร / Admin | ผู้บริหารตอบ "Approved" ทางอีเมล → Admin กด **บันทึก: ผู้บริหารอนุมัติแล้ว** (หรือผู้บริหารที่เป็นสมาชิกกดอนุมัติในแอปเอง) | อนุมัติ / ไม่อนุมัติ |
| 4 | Admin | **ส่งแจ้งฝ่ายจัดซื้อ** ("ได้รับการอนุมัติสั่งซื้ออุปกรณ์ …") → ใส่เลข **PR/PO** (ตัดงบลง `pr_po_log` ตาม PC × Budget × เดือน) | ออก PR/PO แล้ว |
| 5 | Admin | **รับของเข้าแล้ว** | รับของแล้ว |
...
ค่าตั้งต้นของอีเมล (ผู้อนุมัติ, สำเนา, ฝ่ายจัดซื้อ, ชื่อทีม, ลายเซ็น) ตั้งที่ Admin → **ตั้งค่าอีเมลขออนุมัติ** (ชีต `settings`)
...
- คงเหลือ = งบเดือนนั้น (Revise Budget ถ้ามี ไม่งั้น Budget) − Actual − รายการในคำขอที่รอตรวจ/รออนุมัติ/อนุมัติแล้ว
  (นับตาม Budget + เดือนของแต่ละรายการ)
- ชีตในฐานข้อมูล: `members`, `requests`, `request_items`, `budget_master`, `settings`; คำขอที่ออก PR/PO แล้วบันทึกลง
...
src/
  ...
  Requests.js     workflow คำขอ: เลือก Budget รายรายการ/Over Budget → Admin ตรวจ → PR/PO → รับของ + Budget + เตือนซ้ำ
