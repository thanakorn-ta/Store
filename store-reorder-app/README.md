...
|---|---|---|
| เมนู | หน้าแรก · **สั่งซื้อตาม Budget** · คำขอของฉัน · งบประมาณ · ติดตามการส่งของ · รายการแนะนำจาก Store | ภาพรวม · **ตรวจคำขอสั่งซื้อ** · สั่งซื้อตาม Budget · งบประมาณ · ติดตามการส่งของ · รายการแนะนำ · ข้อมูล Store · ตารางรวม · สมาชิก & ตั้งค่า · ผู้ช่วย AI |
...
| 5 | Admin | **ส่งแจ้งฝ่ายจัดซื้อ** ("ได้รับการอนุมัติสั่งซื้ออุปกรณ์ …" แนบไฟล์เหมือนกัน) → ใส่เลข **PR/PO รายชิ้น** ที่การ์ดคำขอ (ติ๊กทีละรายการ หรือติ๊กหลายรายการที่ใช้เลขเดียวกัน) พร้อม **กำหนดส่ง (ประมาณ)** — รายการที่เพิ่งได้เลข PO จะตัดงบลง `pr_po_log` ตาม PC × Budget × เดือน ทันที | ออก PR/PO แล้ว (เมื่อทุกรายการมีเลข PO) |
| 6 | Admin | ติ๊กรายการที่ของมาถึง + **ใส่วันที่รับของ** (คำขอเป็น "รับของแล้ว" เมื่อของมาครบทุกรายการ) | รับของแล้ว |
...
- แท็บ **ติดตามการส่งของ** (ทุกคน): ของที่ออก PR/PO แล้ว — ยังไม่ได้รับ / เลยกำหนดส่ง / รับแล้ว · ระยะส่งเฉลี่ย-กลาง-ช้าสุด ·
  % ส่งตรงเวลา · **วิเคราะห์**: ควรเผื่อเวลากี่วัน (p90 ของที่เคยรับจริง) · PC ที่ของมาช้าที่สุด · ของที่ต้องตามตอนนี้ ·
  แยกตามเลข PR/PO · ส่งออก .xlsx ได้ · ระบบใช้ตัวเลขชุดเดียวกันนี้เติม "กำหนดส่ง (ประมาณ)" ให้อัตโนมัติ (วัน PO + p90)
- ชีต `request_items` เก็บ `pr_no`, `po_no`, `po_at`, `eta_date`, `received_at` รายชิ้น (`src/Delivery.js`) —
  ของคำขอเดียวกันจึงอยู่คนละ PR/PO และมาคนละวันได้ · `issuePo` แบบเดิม (ทั้งคำขอเลขเดียว) ยังใช้ได้และไม่ตัดงบซ้ำ
- หน้า **รายการแนะนำจาก Store**: ทุกแถวที่ติ๊กจะมีช่อง **"ใส่ในงบ"** ต่อท้าย บอกเป็นเงินเท่าไร เข้า Budget ไหน เดือนไหน ·
  ใต้ตารางมีสรุปทุก Budget ที่ติ๊กไว้ (ยอดรวม · งบเหลือ · คงเหลือหลังสั่ง · แดงเมื่อเกินงบ)
- หน้า **งบประมาณ** และ **สั่งซื้อตาม Budget** แสดงเฉพาะหมวดที่ Store ใช้สั่งซื้อ: **ค่าซ่อมแซมบำรุงรักษา · ค่าวัสดุสิ้นเปลืองใช้ไป ·
  ค่าซ่อมภาพโฆษณา** (หมวดอื่น เช่น ค่าไฟฟ้าป้ายโฆษณา ถูกซ่อน) · ติ๊ก "แสดงทุกหมวดงบ" ที่หน้างบประมาณเพื่อดูทั้งหมด ·
  กรองจาก**ชื่อหมวด** (`GL_WORDS` ใน `web/app.js`) ไม่ใช่เลข GL เพราะแต่ละบริษัทใช้เลขไม่เหมือนกัน · ถ้าไม่มีหมวดไหนตรงเลย ระบบจะแสดงทั้งหมดแทนหน้าว่าง
...
src/
  ...
  Delivery.js     PR/PO + กำหนดส่ง + วันรับของ รายชิ้น (ตัดงบเมื่อได้เลข PO)
